...
for m in 5e-07
	...
	for r in 1e-8
		...
		for p in  500 
			do
			mkdir slim_output_files_m${m}_r${r}_p${p}
				## models chromosome with coding and non-coding regions
				slim \
					-d POP_SIZE=$p \
					-d MUTATION_RATE=$m \
					-d RECOMB_RATE=$r \
					-d "OUT_PATH='/scratch/${USER}_${PROJ}/slim_output_files_m${m}_r${r}_p${p}/'" \
					/home/$USER/roh_param_project/$PROJ/scripts/chrom_w_struct_and_evo.slim
			done
		done
	done

mail -s 'SLiM run finished - Starting BCFtools' avrilharder@gmail.com <<< 'SLiM run finished - Starting BCFtools'
...
mail -s 'SLiM run finished - submit Rviz' kirkseykb1@appstate.edu <<< 'SLiM run finished'